...
class Ferrofluid {
  ...
  init() {
    ...
    if (!container) { console.warn('Ferrofluid: container not found'); return; }
    ...
    const shimmer = opts.shimmer ?? 1.5;
    ...
    const mouseRadius = opts.mouseRadius ?? 0.35;
    ...
    if (mouseInteraction) {
      ...
      this._pointerHandler = onPointerMove;
    }
    ...
    const loop = (t) => {
      ...
      if (!paused && this.program && this.mesh) {
        try {
    ...
    this.destroy = () => {
      ...
      if (this._pointerHandler && canvas) canvas.removeEventListener('pointermove', this._pointerHandler);
      if (this.ro) this.ro.disconnect();
      if (canvas.parentElement === container) container.removeChild(canvas);
      if (this.program) { try { this.program.remove(); } catch(e) {} }
      if (this.geometry) { try { this.geometry.remove(); } catch(e) {} }
      if (this.mesh) { try { this.mesh.remove(); } catch(e) {} }
